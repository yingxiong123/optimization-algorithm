...
class Individual():
    def __init__(self):
        ...
        self.objective = defaultdict()    #如果字典没有key，返回一个默认值,返回的是{f1:5,f2:9}
    ...
    def __lt__(self, other):
        if isinstance(other, Individual):
            ...
            for i in range(len(v1)):
                if v1[i] > v2[i]:
                    return 0 
            else:   #所有目标都小于other,说明self支配other
                return 1
        return NotImplemented  # 避免与其他类型进行无意义比较
...
def main():
    ...
    #先通过二元锦标赛选择父代，进行交叉变异，产生100个子代，一个200个
...
def crowding_distance_assignment(L):
    """ 传进来的参数应该是L = F(i)，类型是List,元素是individual"""
